"""
eval_mlp_ne_large.py — Evaluate a trained MLP against 24 NE-strategy background traders.

Large-scale version of eval_mlp_ne.py with 24 background agents (matching the
equilibrium experiment and training setup).

Phase 3 of the large-scale MLP experiment:
  1. Load a pre-trained SAC model (from train_mlp_ne_large.py)
  2. Run it as a deviator against 24 background agents all playing strategy S*
...
Usage
-----
    python eval_mlp_ne_large.py --ne-strategy 0 --model runs/sac_zi_ne_large_s0_v1/best_model
    python eval_mlp_ne_large.py --ne-strategy 0 --model runs/sac_zi_ne_large_s0_v1/best_model --num-runs 2000
"""
...
BASE_SEED     = 42
SHADE_RANGE   = [0, 600]
# Normalizers must match train_mlp_ne_large.py training normalizers
NORMALIZERS   = {"fundamental": 1e5, "invt": 10, "reward": 1e3, "pv": 5e5}


# ---------------------------------------------------------------------------
# SACDeviator (same as eval_mlp_ne.py but with updated n_bg)
# ---------------------------------------------------------------------------
...
def _run_mlp_vs_ne(args):
    ...
    CRN: seed = base_seed + run_idx (same as ZI deviators for comparability)
    """
...
def _run_zi_vs_ne(args):
    ...
    CRN: seed = base_seed + run_idx (dev_idx excluded for CRN across deviators)
    """
...
def run_evaluation(ne_idx: int, model_path: str, num_runs: int = DEFAULT_RUNS,
    ...
    print(f"\nPHASE 3 (LARGE): MLP vs NE Strategy Evaluation")
    print(f"  NE strategy    : S{ne_idx}  {ne_strat}")
    print(f"  Model          : {model_path}")
    print(f"  Background     : {ENV['n_bg']} agents")
...
def display_results(ne_idx: int, zi_means: np.ndarray, zi_stds: np.ndarray,
    ...
    print("\n" + "=" * 80)
    print(f"RESULTS: Deviator profit vs S{ne_idx} background  ({ne_strat})  [24 BG agents]")
...
def parse_args():
    ...
    p.add_argument("--output", type=str, default=None,
                   help="Output CSV path (default: eval_mlp_ne_large_s<idx>.csv)")
    return p.parse_args()
...
def main():
    ...
    # Save results to CSV
    out = args.output or f"eval_mlp_ne_large_s{args.ne_strategy}.csv"
    rows = []
