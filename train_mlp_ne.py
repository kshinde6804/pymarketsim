"""
train_mlp_ne.py — Train MLP agent against a fixed NE background strategy.

Unlike train_mlp.py (which randomizes the background strategy each episode),
this script trains the agent in an environment where ALL 15 background traders
consistently play the Nash Equilibrium strategy found in the large-scale ZI×ZI
experiment.  This produces an MLP specialised for that specific market regime.

Usage
-----
    # After finding NE candidate S5 from run_large_zi_zi.py:
    python train_mlp_ne.py --ne-strategy 5

    # With custom timesteps and tag:
    python train_mlp_ne.py --ne-strategy 5 --timesteps 2000000 --tag ne_s5

    # Eval only:
    python train_mlp_ne.py --ne-strategy 5 --eval-only --load runs/sac_zi_ne_s5/best_model

Outputs (under runs/sac_zi_ne_s<idx>_<tag>/)
---------------------------------------------
...
# ---------------------------------------------------------------------------
# Base environment kwargs (match equilibrium experiment)
# ---------------------------------------------------------------------------
...
def evaluate_zi_baseline(ne_strategy: dict, n_episodes: int = 20) -> dict:
    ...
    s = ne_strategy
    ne_shade_mid = (s['shade'][0] + s['shade'][1]) / 2
    strategies = {
...
def plot_learning_curve(run_dir: str, zi_baseline: dict, total_timesteps: int):
    ...
    ax.set_title("SAC MLP Agent — ZIEnv Training vs NE Strategy")
...
def parse_args():
    ...
    p.add_argument("--timesteps", type=int, default=2_000_000,
                   help="Total SAC training steps (default 2,000,000)")
...
def main():
    ...
    run_dir = os.path.join("runs", f"sac_zi_ne_s{args.ne_strategy}_{tag}")
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=max(args.eval_freq * 5, 50_000),
        save_path=os.path.join(run_dir, "checkpoints"),
        name_prefix="sac_zi_ne",
        verbose=0,
    )
    ...
    print(f"\nNext step — evaluate MLP vs 15 NE background agents:")
    print(f"  python eval_mlp_ne.py --ne-strategy {args.ne_strategy} --model {best_model_path}")
